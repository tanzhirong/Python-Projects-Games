...
class Sprite:
    ...
    def collide(self, thing):
        if (dist(self.get_position(), thing.get_position()) < 
            (self.get_radius() + thing.get_radius())):
            return True
        else:
            return False
...
# for explosion 
def group_collide(sprite_group, thing):
    for sprite in set(sprite_group):
        if sprite.collide(thing):
            explode = Sprite(sprite.get_position(), [0, 0], 0, 0, 
...
def group_group_collide(group1, group2):
    for thing in set(group1):
        if group_collide(group2, thing) is True:
            group1.discard(thing)
            return True
    return False 

# mouseclick handlers that reset UI and conditions whether splash image is drawn
def click(pos):
    global started, lives, score, rock_limit
...
def draw(canvas):
    global time, started, lives, score, rock_limit, rock_group
